...
assert.ok(result.prototype.screenCount >= 6 && result.prototype.screenCount <= 16);
...
assert.equal(prototypeSpec.schemaVersion, "2.0");
assert.equal(prototypeSpec.project.type, "ecommerce");
assert.equal(prototypeSpec.screens.some((screen) => screen.id === "admin_catalog"), false);
assert.ok(prototypeSpec.screens.some((screen) => screen.id === "checkout"));
