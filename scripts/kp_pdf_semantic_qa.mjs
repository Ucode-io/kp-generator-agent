        ...
        for (const attr of element.attributes) {
          ...
          if (attr.name === "href") {
            try {
              ...
              const localHttp = target.protocol === "http:" && /^(?:127\.0\.0\.1|localhost|\[::1\])$/i.test(target.hostname);
              if (tagName !== "A" || (!localHttp && target.protocol !== "https:") || target.username || target.password) unsafe.push({ code: "DOM_UNSAFE_MARKUP", severity: "BLOCKER", elementIds: [tagName, attr.name] });
            } catch {
              unsafe.push({ code: "DOM_UNSAFE_MARKUP", severity: "BLOCKER", elementIds: [tagName, attr.name] });
            }
          }
          if (attr.name === "target" && (tagName !== "A" || attr.value !== "_blank")) unsafe.push({ code: "DOM_UNSAFE_MARKUP", severity: "BLOCKER", elementIds: [tagName, attr.name] });
          if (attr.name === "rel" && (tagName !== "A" || attr.value !== "noopener noreferrer")) unsafe.push({ code: "DOM_UNSAFE_MARKUP", severity: "BLOCKER", elementIds: [tagName, attr.name] });
        }
      }
      return unsafe;
    }
